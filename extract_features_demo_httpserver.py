...
similarity_smooth = lambda x, y, z, u: (x * y) + z - u
...
def load_dataset(filename):
    ...
    with open(filename, 'rt') as reader:
        for line in reader:
            ...
            sents.append(fields[0])
            fea = np.asarray([float(x) for x in fields[1:]])
            feas.append(fea)

    return sents, np.asarray(feas)
...
def similarity_distance(s1, s2, wv1, wv2):
    u = levenshtein_distance(s1, s2)
    g = cosine(wv1, wv2)

    if u >= 0.99:
        r = 1.0
    elif u > 0.9:
        r = similarity_smooth(g, 0.05, u, 0.05)
    elif u > 0.8:
        r = similarity_smooth(g, 0.1, u, 0.2)
    elif u > 0.4:
        r = similarity_smooth(g, 0.2, u, 0.15)
    elif u > 0.2:
        r = similarity_smooth(g, 0.3, u, 0.1)
    else:
        r = similarity_smooth(g, 0.4, u, 0)
...
def main(_):
    ...
    sents, X = load_dataset('sentence_fea_out.txt')
    ...
    while True:
        ...
        print(text)
        distances, points = tree.query(np.array([sentence_wc]), 11)
        distances = distances[0]
        points = points[0]
        number = 0
        for (d, idx) in zip(distances, points):
            if number > 0:
                edit_dist = levenshtein_distance(text.lower(), sents[idx].lower())
                sim_score = similarity_distance(text, sents[idx], sentence_wc, X[idx])
                print('KNN_%d ==> %s, dist=%f, sim_score %f, edit_dist=%f' %(number, sents[idx], d, sim_score, edit_dist))
            number += 1
